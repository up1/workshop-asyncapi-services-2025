// Package "api" provides primitives to interact with the AsyncAPI specification.
//
...
type AppSubscriber interface {
	// OnOrderCreatedOperationReceived receive all Order messages from OrderCreated channel.
	OnOrderCreatedOperationReceived(ctx context.Context, msg OrderMessage) error
}
...
func (c *AppController) SubscribeToAllChannels(ctx context.Context, as AppSubscriber) error {
	...
	if err := c.SubscribeToOnOrderCreatedOperation(ctx, as.OnOrderCreatedOperationReceived); err != nil {
		return err
	}
...
func (c *AppController) UnsubscribeFromAllChannels(ctx context.Context) {
	c.UnsubscribeFromOnOrderCreatedOperation(ctx)
}

// SubscribeToOnOrderCreatedOperation will receive Order messages from OrderCreated channel.
//
...
func (c *AppController) SubscribeToOnOrderCreatedOperation(
	ctx context.Context,
	fn func(ctx context.Context, msg OrderMessage) error,
) error {
	// Get channel address
	addr := "order_created"
	...
	go func() {
		for {
			...
			stop, err := c.listenToOnOrderCreatedOperationNextMessage(addr, sub, fn)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
...
func (c *AppController) listenToOnOrderCreatedOperationNextMessage(
	addr string,
	...
	if err := c.executeMiddlewares(msgCtx, &acknowledgeableBrokerMessage.BrokerMessage, func(middlewareCtx context.Context) error {
		// Process message
		msg, err := brokerMessageToOrderMessage(acknowledgeableBrokerMessage.BrokerMessage)
		if err != nil {
			return err
		}
...
// UnsubscribeFromOnOrderCreatedOperation will stop the reception of Order messages from OrderCreated channel.
// A timeout can be set in context to avoid blocking operation, if needed.
func (c *AppController) UnsubscribeFromOnOrderCreatedOperation(
	ctx context.Context,
) {
	// Get channel address
	addr := "order_created"
...
// SendToOnOrderCreatedOperation will send a Order message on OrderCreated channel.
//
// NOTE: for now, this only support the first message from AsyncAPI list.
// If you need support for other messages, please raise an issue.
func (c *UserController) SendToOnOrderCreatedOperation(
	ctx context.Context,
	msg OrderMessage,
) error {
	// Set channel address
...
// Message 'OrderCreatedMessageFromOrderCreatedChannel' reference another one at '#/components/messages/Order'.
...
type OrderMessagePayload struct {
	...
	Items []ItemFromItemsPropertyFromOrderMessagePayload `json:"items,omitempty"`
...
type ItemFromItemsPropertyFromOrderMessagePayload struct {
...
// OrderMessage is the message expected for 'OrderMessage' channel.
...
// brokerMessageToOrderMessage will fill a new OrderMessage with data from generic broker message
func brokerMessageToOrderMessage(bMsg extensions.BrokerMessage) (OrderMessage, error) {
	var msg OrderMessage
...
// toBrokerMessage will generate a generic broker message from OrderMessage data
func (msg OrderMessage) toBrokerMessage() (extensions.BrokerMessage, error) {
...
const (
	// OrderCreatedChannelPath is the constant representing the 'OrderCreatedChannel' channel path.
	OrderCreatedChannelPath = "order_created"
)

// ChannelsPaths is an array of all channels paths
var ChannelsPaths = []string{
	OrderCreatedChannelPath,
}
